...
async def send_error_to_endpoint(endpoint_url: str, board_id: Optional[str], request: Request, exception: Exception):
    ...
    try:
        ...
        # Build payload - only send if boardId is available
        if not board_id:
            logger.error('[EXCEPTION HANDLER] Cannot build payload: boardId is None or empty')
            return
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    if runtime_error_endpoint_url:
        if board_id:
            ...
            logger.error('[EXCEPTION HANDLER] Cannot send error to endpoint: boardId is NULL. Endpoint requires boardId.')
            logger.error(f'[EXCEPTION HANDLER] Request path: {request.url.path}, Host: {request.headers.get("host", "N/A")}')
            logger.error(f'[EXCEPTION HANDLER] BOARD_ID env var: {os.getenv("BOARD_ID", "NOT SET")}')
            logger.error(f'[EXCEPTION HANDLER] RUNTIME_ERROR_ENDPOINT_URL: {runtime_error_endpoint_url}')
    else:
